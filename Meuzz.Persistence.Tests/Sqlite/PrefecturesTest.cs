...
namespace Meuzz.Persistence.Tests.Sqlite
{
    ...
    public class PrefecturesTest
    {
        ...
        public void TestLoadByIdAndEdgesWithJoins()
        {
            ...
            var pref = edges.First().Item2;
            Assert.Equal("èHìc", pref.Name);
            var pref1 = edges.Skip(1).First().Item2;
            Assert.Equal("éRå`", pref1.Name);
            var pref2 = edges.Last().Item2;
            Assert.Equal("ïüìá", pref2.Name);
        }
        ...
        public void TestLoadByIdAndEdgesWithJoinsAndSelect()
        {
            ...
            Assert.Equal("èHìc", prefs.First().Name);
            Assert.Equal("éRå`", prefs.Skip(1).First().Name);
            Assert.Equal("ïüìá", prefs.Last().Name);
        }
